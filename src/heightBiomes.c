 ...
 * 
 * @param noiseLoc location on the map to sample heightBiome at
 * 
...
HeightBiome* getHeightBiomeForPoint(SPVec3 noiseLoc) {
    ...
    BiomeOdds oddsThresholds[biomeRowSize];
    double threshold = 1;
    ...
    for(int x = -1; x <= 1; x++) {
        for(int y = -1; y <= 1; y++) {
            for(int z = -1; z <= 1; z++) {
                ...
                BiomeOdds biomeOfChoice = oddsThresholds[0];
                for(int i = 1; i < biomeRowSize; i++) {
                    BiomeOdds oddsThreshold = oddsThresholds[i];
    ...
    int biomeWeightCounts[HEIGHT_BIOME_COUNT];
    ...
    for(int i = 0; i < 27; i++) {
        HeightBiome biomeBlend = biomeBlends[i];
    ...
    HeightBiome *biomes = malloc(sizeof(HeightBiome) * (biomeSize+1));

    // A dirty hack to provide the length is to add the length as first element in the array, abusing HeightBiome but oh well
    HeightBiome length = { biomeSize, 0 };
    biomes[0] = length;
